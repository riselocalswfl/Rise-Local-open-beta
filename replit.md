...
- **Simplified Authentication Flow** (November 2025): The platform now provides a streamlined signup experience to reduce vendor confusion. The `/join` page serves as the primary entry point with a vendor-focused design featuring a prominent "Start Selling on Rise Local" CTA button. A secondary "Sign up as a customer" link preserves buyer account creation. The authentication flow routes users based on `intended_role` query parameter: vendors go through `/api/login?intended_role=vendor` → Replit Auth → `/onboarding`, while buyers use `/api/login?intended_role=buyer` → Replit Auth → home/profile. **Legacy cleanup**: Deleted 16 legacy pages including 6 signup pages (Login, Signup, BuyerSignup, VendorSignup, RestaurantSignup, ServiceProviderSignup), 4 type-specific onboarding pages (ShopOnboarding, DineOnboarding, ServicesOnboarding, VendorOnboarding), 3 dashboard pages (RestaurantDashboard, ServiceProviderDashboard), and 3 event pages (Events, MyEvents, consolidated into EventsLayout). All legacy routes (`/login`, `/signup`, `/join/vendor`, etc.) redirect to `/join`. The homepage hero section features "Start Selling on Rise Local" button linking directly to `/join`.
- **Unified Vendor Onboarding** (November 2025): Single universal onboarding flow at `/onboarding` serves all vendor types. The 4-step wizard guides users through business profile creation: (1) Basic Info, (2) Brand & Values, (3) Payment & Fulfillment, (4) Review & Submit. Vendors self-select their `vendorType` ("shop"/"dine"/"service") at the start, which automatically sets initial capabilities: Shop sets `{products: true, services: false, menu: false}`, Dine sets `{products: false, services: false, menu: true}`, Services sets `{products: false, services: true, menu: false}`. Auto-save system with 2-second debounced triggers creates draft profiles automatically. All submissions include `credentials: "include"` for session cookie handling. Onboarding uses `/api/vendors/draft` endpoint; backend transparently creates unified vendors with proper capabilities.
...
- **Services Tab Display** (November 2025): The `/services` page displays individual service offerings from the `serviceOfferings` table, NOT service provider profiles. Each service offering card shows the service name, description, pricing (fixed/hourly/quote), duration, tags, and embedded provider information (name, logo, location, verification status). Cards link to the provider's profile page via "View Provider" button. The `/api/services` endpoint returns active service offerings joined with vendor data via `getAllServiceOfferingsWithProvider()`. Service provider profiles appear on the `/vendors` "All Vendors" tab alongside shops and restaurants.
...
- **Customer-Facing Vendor Pages** (November 2025): All customer-facing vendor profile pages (`/vendor/:id`, `/restaurant/:id`, `/services/:id`) implement capability-based rendering. Pages use the unified `/api/vendors/:id` endpoint and extract vendor-specific data from JSON fields (`restaurantDetails`, `serviceDetails`). Capability gates control both data fetching AND UI rendering: Menu section (fetch + render) only appears if `capabilities.menu = true`, Services section only if `capabilities.services = true`, Products section only if `capabilities.products = true`. CTAs (e.g., "View Menu" button) are also gated by capabilities. This ensures vendors only display features they've enabled, preventing empty sections and unnecessary API calls. Legacy routes (`/restaurant/:id`, `/services/:id`) maintain backward compatibility while using the unified vendor system.
- **Vendor Listing System** (November 2025): The `getAllVendorListings()` function fetches exclusively from the unified `vendors` table, filtering for `profileStatus = "complete"` to ensure only finished profiles appear on listing pages (`/vendors`, `/products`, `/eat-local`, `/services`). Returns `UnifiedVendorListing[]` with vendorType-based routing. Categories field returns empty array per platform simplification. Legacy signup flows (`VendorSignup.tsx`, `RestaurantSignup.tsx`) contain unused category selectors but are deprecated in favor of current onboarding flows which don't use categories.
...
- **Application Routes**: 
  - **Public routes**: `/` (homepage), `/products`, `/vendors`, `/eat-local`, `/services`, `/events`, `/vendor/:id`, `/restaurant/:id`, `/services/:id`, `/event/:id`, `/join`
  - **Protected buyer routes** (require authentication): `/cart`, `/checkout`, `/orders`, `/order-confirmation`, `/profile`, `/messages`
  - **Protected vendor routes** (require authentication + vendor role): `/onboarding`, `/dashboard`
  - **Legacy redirects**: `/login` → `/join`, `/signup` → `/join`, `/join/vendor` → `/join`, `/join/restaurant` → `/join`, `/join/service-provider` → `/join`
