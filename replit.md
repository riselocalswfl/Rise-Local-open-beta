...
- **Multi-Role User System**: Users can have both admin and vendor privileges simultaneously via boolean flags:
  ...
  - The legacy `role` field is preserved for backward compatibility - authorization checks use `isAdmin || role === 'admin'` pattern
  - Admin+vendor users see both admin and vendor navigation links in the navigation menu
  - Existing single-role users continue to work unchanged (flags were backfilled from role values)
