...
- **Vendor Architecture**: All vendor types (shops, restaurants, service providers) are unified under a `vendors` table. `menuItems` and `services` tables link to vendors via `vendorId`.
...
- **Vendor Onboarding**: Three distinct 4-step onboarding flows (`/onboarding/shop`, `/onboarding/dine`, `/onboarding/services`) are tailored for each business type, integrating with Replit Auth callbacks and collecting type-specific details.
