...
- **Mobile-First Discover Page**: The `/discover` page is the new consumer home featuring a location bar, horizontally-scrollable category pills (All, Meals, Bread & pastries, Groceries, Drinks, Treats), and two deal sections ("Top picks near you", "Pick up now") with horizontal card scrolling. Currently uses mock data. Bottom tab navigation includes Discover, Browse, Favorites, and Profile tabs.
...
- **Application Routes**:
  - **Public Routes**: `/`, `/discover`, `/browse`, `/favorites`, `/auth`, `/products`, `/vendors`, `/vendor/:id`, `/services`, `/eat-local`, `/live-local`, `/events`, `/events/:id`, `/spotlight`, `/cart`.
