...
- `text-page-title`: Large page headers (1.5rem/24px, weight 600)
- `text-section-header`: Card/section titles (1.125rem/18px, weight 600)
- `text-deal-title`: Deal/item names (1rem/16px, weight 600)
- `text-business-name`: Business/vendor names (0.875rem/14px, weight 600)
- `text-body`: Body copy and descriptions (0.9375rem/15px, weight 400)
- `text-meta`: Metadata, timestamps, secondary info (0.8125rem/13px, weight 400)
- `text-label`: Form labels (0.875rem/14px, weight 500)
- `text-input`: Form input text (0.875rem/14px, weight 400)
- `text-button`: Button text (0.875rem/14px, weight 500)
- `text-meta-emphasis`: Emphasized metadata (0.8125rem/13px, weight 500)

The `--muted-foreground` CSS variable is set to near-black (25% lightness) for high contrast secondary text.
...
- **Auto-Publish Deals**: New deals are automatically published upon creation (status='published'). Vendors see a "Deal published!" toast and the deal immediately appears on Discover/Browse. Vendors can still pause/unpublish deals at any time using the Pause button, and re-publish paused deals. Deal statuses: `published` (visible to shoppers), `paused` (hidden from public), `draft` (legacy).
- **B2C Messaging System**: Implements a consumer-to-business messaging system with optional deal context. Features include "Message Business" buttons, vendor message viewing and replies in the Dashboard, and in-app notifications. The messaging UI displays participant avatars (consumer profile photos with initials fallback for vendors, business logos for consumers) in both conversation lists and chat threads.
  - **Database Tables**: `conversations` (conversation metadata with consumerId, vendorId, dealId) and `conversation_messages` (individual messages with senderId, content, isRead, createdAt).
  - **Frontend Routes**: `/messages` (MessagesPage.tsx - conversation list), `/messages/:conversationId` (ConversationPage.tsx - chat thread).
  - **API Endpoints**: POST `/api/b2c/conversations/start` (create/get conversation), GET `/api/b2c/conversations` (list user's conversations), GET/POST `/api/b2c/conversations/:conversationId` (read/send messages), GET `/api/b2c/unread-count` (unread count).
  - **Realtime**: ConversationPage uses 5-second polling (`refetchInterval: 5000`) for new messages.
  - **Legacy P2P Messaging**: A separate `messages` table exists for direct user-to-user messaging at `/dm` route (Messages.tsx), using `/api/messages` endpoints. This is deprecated in favor of B2C messaging.
  - **Membership Check Utility**: Centralized `checkIsPassMember(user)` function in `client/src/lib/authUtils.ts` handles membership status with proper invalid date validation using `Number.isNaN()` and `getTime()` checks.
- **Messaging Email Notifications**: An email job queue system schedules delayed email notifications (5-minute delay) when users receive new messages and haven't viewed them. Rate limiting prevents duplicate emails within 30 minutes per conversation. Viewing a conversation cancels pending email jobs. The `email_jobs` table tracks job status, and a background worker processes pending jobs on a 1-minute interval. User email preferences are stored in `emailMessageNotifications` field.
- **UserAvatar Component**: A reusable avatar component (`client/src/components/UserAvatar.tsx`) displays profile photos with initials fallback. Supports sizes: xs (24px), sm (28px), md (32px), lg (40px), xl (48px). Generates initials from firstName/lastName or falls back to first character of displayName.
- **Public Profile Endpoint**: GET `/api/users/:id` returns limited public profile data (id, displayName, profileImageUrl, firstName, lastName) for displaying sender info in messages and notifications. Sensitive user data is not exposed.
...
- **Redemption Frequency Controls**: Vendors can set per-deal redemption limits when creating/editing deals using a simple radio group with three options: "Once per week" (7-day rolling window), "Once per month" (30-day rolling window), or "Unlimited redemptions". The backend validates these limits during claim attempts and blocks redemptions with friendly error messages showing days remaining. Deal cards display a refresh icon with "1x/week" or "1x/month" labels in the metadata section; unlimited deals show no indicator. The Deal Detail page shows the same information as an outline badge.
