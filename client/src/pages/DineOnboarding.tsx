...
export default function DineOnboarding() {
  ...
  // Auto-save state
  const [draftRestaurantId, setDraftRestaurantId] = useState<string | null>(null);
  ...
  // Load draft restaurant on mount
  useEffect(() => {
    const loadDraft = async () => {
      try {
        const response = await fetch("/api/restaurants/draft", {
          credentials: "include",
        });
        
        if (!response.ok) {
          console.error("Failed to fetch draft restaurant");
          return;
        }
        ...
        if (draft && draft.id) {
          console.log("Loaded draft restaurant:", draft.id);
          setDraftRestaurantId(draft.id);
        ...
        console.error("Error loading draft restaurant:", error);
      } finally {
  ...
  const autoSave = useCallback(async (data: any, formType: 'step1' | 'step2' | 'step3') => {
    ...
    try {
      ...
      const response = await fetch(`/api/restaurants/${draftRestaurantId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
  ...
  const handleStep1Submit = async (data: any) => {
    ...
    try {
      if (!draftRestaurantId) {
        ...
        setDraftRestaurantId(restaurant.id);
        console.log("Created draft restaurant:", restaurant.id);
      } else {
  ...
  const handleStep4Submit = async (connectStripe: boolean = false) => {
    ...
    try {
      if (!draftRestaurantId) {
        throw new Error("No draft restaurant found. Please start from the beginning.");
      }

      // Mark profile as complete
      const response = await fetch(`/api/restaurants/${draftRestaurantId}/complete`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      });
          ...
          <div className="mt-2 flex items-center justify-end gap-2">
            ...
            {saveStatus === "saved" && (
              <>
                ...
                <span className="text-sm text-green-600 dark:text-green-500">Draft saved</span>
              </>
            )}
            {saveStatus === "error" && (
              <span className="text-sm text-red-600 dark:text-red-500">Failed to save draft</span>
            )}
          </div>
        </div>
